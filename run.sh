#!/bin/bash

# 配置区域
# 1. SSH 配置
ALLOW_ROOT_LOGIN=true  # 是否允许 root 用户通过 SSH 登录，设置为 true 或 false

# 2. 公钥配置
PUBLIC_KEY=" bab-mac@baby.com"  # 请将此处替换为你的公钥内容

# 3. Git 仓库配置
REPO_URL="https://github.com/suminerProxy/agent.git"  # 请将此处替换为你的 Git 仓库地址

# 4. 脚本配置
START_SCRIPT="start.sh"  # 需要运行的脚本名称

# 检查是否以 root 用户运行脚本
if [ "$(id -u)" -ne 0 ]; then
  echo "请使用 root 用户运行此脚本。"
  exit 1
fi

# 1. 根据配置修改 SSH 配置，允许 root 用户直接登录
if [ "$ALLOW_ROOT_LOGIN" == "true" ]; then
  echo "修改 SSH 配置，允许 root 用户直接登录..."
  sed -i 's/^#PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config
  systemctl restart sshd
else
  echo "不允许 root 用户通过 SSH 直接登录。"
fi

# 2. 添加公钥到 root 用户的 authorized_keys
echo "添加公钥到 root 用户的 authorized_keys..."
mkdir -p /root/.ssh
echo "$PUBLIC_KEY" >> /root/.ssh/authorized_keys
chmod 600 /root/.ssh/authorized_keys

# 3. 克隆指定的 Git 仓库
echo "克隆 Git 仓库: $REPO_URL ..."
cd /root
git clone "$REPO_URL"

# 4. 进入仓库目录并执行 start.sh 脚本
cd "$(basename "$REPO_URL" .git)"
if [ -f "$START_SCRIPT" ]; then
  echo "进入目录并执行 $START_SCRIPT ..."
  bash "$START_SCRIPT"
else
  echo "未找到 $START_SCRIPT 脚本，确保仓库中存在该文件。"
  exit 1
fi

echo "脚本执行完成！"
